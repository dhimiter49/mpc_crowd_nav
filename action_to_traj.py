...
for i in range(0, len(motion_data), mult_plan):
    ...
    for a in actions:
        ...
        trajectories[-1].append(env.unwrapped.current_pos)
...
for i, traj in enumerate(trajectories):
    traj_arrs['traj' + str(i)] = np.array(traj)
name = motion_data_path.split('/')[-1].split('.')[0]
path = '/'.join(motion_data_path.split('/')[:-1])
np.savez(path + "/traj_" + name, **traj_arrs)
